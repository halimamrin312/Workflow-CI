...
def train_model(n_estimators, max_depth, min_samples_split):
    ...
    # Selalu set tracking URI ke path relatif agar artifact disimpan
    # di direktori kerja runner (bukan path absolut mesin lokal).
    ...
    if not is_in_mlflow_run:
        ...
        if dagshub_username and dagshub_token:
            ...
            tracking_uri = f"https://dagshub.com/{dagshub_username}/mlops-cardio-disease.mlflow"
            mlflow.set_tracking_uri(tracking_uri)
            os.environ["MLFLOW_TRACKING_USERNAME"] = dagshub_username
            os.environ["MLFLOW_TRACKING_PASSWORD"] = dagshub_token
            mlflow.set_experiment("Cardio-Disease-Retraining-CI")
        else:
            print("No DagsHub credentials found. Logging to local mlruns...")
            mlflow.set_experiment("Cardio-Disease-Retraining-Local")

    # 2. Memuat dataset preprocessing
    train_path = 'cardio_preprocessing/train_processed.csv'
    test_path = 'cardio_preprocessing/test_processed.csv'

    if not os.path.exists(train_path):
        raise FileNotFoundError("Data preprocessing tidak ditemukan di folder cardio_preprocessing.")
